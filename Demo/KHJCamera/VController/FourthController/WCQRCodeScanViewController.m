...
@interface WCQRCodeScanViewController () <SGQRCodeScanManagerDelegate, SGQRCodeAlbumManagerDelegate>
...
@implementation WCQRCodeScanViewController
...
- (void)dealloc
{
    NSLog(@"WCQRCodeScanViewController - dealloc");
    ...
    if (metadataObjects != nil && metadataObjects.count > 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self.indexP == 1) {
                ...
                [[NSNotificationCenter defaultCenter] postNotification:[NSNotification notificationWithName:recieveShare_Noti object:nil userInfo:dic]];
                [self.navigationController popViewControllerAnimated:YES];
            }
        });
    }
...
- (UILabel *)promptLabel
{
    if (!_promptLabel) {
        ...
        CGFloat promptLabelW = SCREENWIDTH;
