...
#import "DeNetSettingVController.h"
#import "NetCell.h"
#import "KHJAllBaseManager.h"
#import "ZQAlterField.h"
#import "DeDviceAndHot_DConnectVController.h"

@interface DeNetSettingVController ()<UITableViewDelegate,UITableViewDataSource>
{
    UILabel *showLabel1;
    UILabel *showLabel2;
    UILabel *showLabel3;
...
@implementation DeNetSettingVController
...
{
    if (!popView) {
        popView = [[UIView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2-70, 58, SCREEN_WIDTH/2+50, 80+1)];
...
//                [self getwifiList];//这个地方改成，
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                DeDviceAndHot_DConnectVController *adHotVC = [[DeDviceAndHot_DConnectVController alloc] init];
                adHotVC.isAP = YES;
                [[NSUserDefaults standardUserDefaults] setValue:[KHJDecEnc aesDecrypt:self.myInfo.devicePwd] forKey:@"APConnectPWD"];
...
- (void)addShadow
{
    ...
    [[DeHub shareHub] showText:KHJLocalizedString(@"loading", nil) addToView:backgroundView type:_lightGray];
    //   设置超时，以防设备断开，一直请求
    ...
    [self startTimerWithSeconds:10 endBlock:^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([weakNetworkTypeLab.text isEqualToString:@""]) {
                // 如无法获取网络模式，请检查设备是否掉线
                [[DeToast share] showOKBtnToastWith:KHJLocalizedString(@"tips", nil) content:KHJLocalizedString(@"checkNetworkStatus", nil)];
            }
            [DeHub shareHub].hud.hidden = YES;
...
- (void)setMyTable
{
    mmTable             = [[UITableView alloc] initWithFrame:CGRectMake(0, 88 + 10 + 1, SCREEN_WIDTH, SCREEN_HEIGHT - 165) style:UITableViewStylePlain];
...
- (void)setHeader
{
    UIView *headView = [[UIView alloc] initWithFrame:CGRectMake(0, 10, SCREEN_WIDTH, 44*2 + 1)];
    [headView setBackgroundColor:[UIColor whiteColor]];
    headView.layer.borderWidth = 0.8;
    headView.layer.borderColor = bgVCcolor.CGColor;
    [self.view addSubview:headView];
    
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, 44, SCREEN_WIDTH, 0.8)];
    lineView.backgroundColor = bgVCcolor;
    [headView addSubview:lineView];
    
    showLabel1              = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH/2, 44)];
    showLabel1.textColor    = [UIColor blackColor];
    showLabel1.text         = [NSString stringWithFormat:@"    %@",KHJLocalizedString(@"netMode", nil)];
    
    networkTypeLab               = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2, 0, SCREEN_WIDTH/2 - 60, 44)];
    ...
    UIImageView *imgV   = [[UIImageView alloc] initWithFrame:CGRectMake(SCREEN_WIDTH - 60, 0, 44, 44)];
    imgV.image          = [UIImage imageNamed:@"arrowdown"];
    [headView addSubview:imgV];
    [headView addSubview:networkTypeLab];
    
    showLabel2              = [[UILabel alloc] initWithFrame:CGRectMake(0, 44 + 1, SCREEN_WIDTH/2, 44)];
    ...
    showLabel3                  = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2, 44 + 1, SCREEN_WIDTH/2 - 20, 44)];
...
- (void)popPwdView:(NSString *)wSSID
{
    ...
    alertView.ensureBgColor = KHJCcolor;
...
{
    dispatch_async(dispatch_get_main_queue(), ^{
        [[DeHub shareHub] showText:@"" addToView:self.view type:_default];
    });
    WeakSelf
    KHJBaseDevice *dDevice = [[KHJAllBaseManager sharedBaseManager] searchForkey:self.uuidStr];
    if (dDevice) {
