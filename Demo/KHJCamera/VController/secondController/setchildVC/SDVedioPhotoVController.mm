...
#import "DeSDVedioPhotoVController.h"
...
@interface DeSDVedioPhotoVController ()<UITableViewDelegate,UITableViewDataSource,clickPlayAndDownLoad>
{
    UIButton *leftBtn;
    UIButton *rightBtn;
    UIButton *showButton;
...
@implementation DeSDVedioPhotoVController
...
- (void)viewDidLoad
{
    ...
    if ([[KHJAllBaseManager sharedBaseManager] KHJSingleCheckDeviceOnline:self.uuidStr] != 1) {
        [[DeToast share] showOKBtnToastWith:KHJLocalizedString(@"tips", nil) content:KHJLocalizedString(@"deviceDropLine", nil)];    
    }
    ...
    mTable.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
        //加载更多数据
        [self loadMoreData];
    }];
    [self getData];
...
- (void)setTipLab
{
    ...
    tipLabel.center = CGPointMake(SCREEN_WIDTH/2, SCREEN_HEIGHT/2-64);
...
{
    if (!mTable) {
        mTable = [[UITableView alloc] initWithFrame:CGRectMake(0, 40+1, SCREEN_WIDTH, SCREEN_HEIGHT-64-40-1) style:UITableViewStylePlain];
...
{
    if (!downLoadPopView) {
        downLoadPopView = [[DownLoadView alloc] initWithFrame:CGRectMake(0, 1, SCREEN_WIDTH, SCREEN_HEIGHT-1)];
    }
...
- (void)initSetNav
{
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(0, 10, SCREEN_WIDTH*0.52, 44)];
    view.backgroundColor = [UIColor clearColor];
    self.navigationItem.titleView = view;
    
    UISegmentedControl* segment = [[UISegmentedControl alloc]initWithFrame:CGRectMake(0, 4, SCREEN_WIDTH*0.52, 34)];
    ...
    segment.layer.borderColor = KHJCcolor.CGColor;
    segment.layer.cornerRadius = 18;
    segment.backgroundColor = [UIColor whiteColor];
    segment.tintColor = KHJCcolor;
    
    NSDictionary * normalTextAttributes = @{NSFontAttributeName : [UIFont systemFontOfSize:16.0f],NSForegroundColorAttributeName : KHJCcolor};
...
- (void)setDatePicker
{
    dateBgView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 40)];
...
- (void)checkSDFile:(NSString *)dateString//查询视频
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{//点击选择日期，确定按钮
        ...
        [[DeHub shareHub] showText:KHJLocalizedString(@"loading", nil) addToView:self.view type:_lightGray];
...
- (void)backVedio:(NSMutableArray *)mArray andContinue:(bool)b
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [weeKMtable.mj_footer endRefreshing];
    ...
    if (b) {//后面还有
        ...
        CLog(@"后面没有文件");
        dispatch_async(dispatch_get_main_queue(), ^{
            [weeKMtable.mj_footer endRefreshingWithNoMoreData];
        });
    }
    ...
    if ([fArray count] == 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (Theight > SCREEN_HEIGHT-64-40-1-22) {
                Theight = SCREEN_HEIGHT-64-40-1;
            }
            re.size.height =  SCREEN_HEIGHT-64-40-1;
            weeKMtable.frame = re;
            [weeKMtable reloadData];
